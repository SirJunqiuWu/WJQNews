...
//    infoTable.separatorStyle = UITableViewCellSeparatorStyleNone;
    ...
    [infoTable registerClass:[HotNewsCell class] forCellReuseIdentifier:@"HotNewsCell"];
    [self.view addSubview:infoTable];
    
    [infoTable mas_remakeConstraints:^(MASConstraintMaker *make) {
    ...
    infoTable.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingTarget:self refreshingAction:@selector(getMoreData)];
...
- (void)refreshData {
    [self uploadDataWithIsRefresh:YES];
}

- (void)getMoreData {
    [self uploadDataWithIsRefresh:NO];
}

- (void)uploadDataWithIsRefresh:(BOOL)isRefresh {
    if (isRefresh)
    {
        [dataArray removeAllObjects];
    }
    [dataArray addObjectsFromArray:[self ceatData]];
...
- (NSArray *)ceatData {
    ...
    [tpArray addObject:@"近日，据相关数据报告显示，中国服务机器人市场规模在去年(2016年)达到了72.9亿元，同比前年(2015年)增长了，预计到2019年市场规模保守估计有可能将接近152亿元。作为人工智能领域中重要的一环，中国服务机器人行业在未来五年内可能会将会出现几何式增长。此外，去年光是在智能服务机器人领域出现的融资事件就有70多起..."];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    HotNewsCell *cell = (HotNewsCell *)[tableView dequeueReusableCellWithIdentifier:cellID];
    cell.title = dataArray[indexPath.row];
    return cell;
}

- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
//    HotNewsCell *cell = (HotNewsCell *)[self tableView:tableView cellForRowAtIndexPath:indexPath];
//    return [cell getLayoutCellHeightWithFlex:8];
    NSString *title = dataArray[indexPath.row];
    return [HotNewsCell getCellHeightWithTitle:title];
}
